    ...
    new Map(
      buses.map((b) => [
        JSON.stringify({
          ...
          price: b.price,
          provider: b.provider,
          providerBusId: b.providerContext?.busId,
        }),
        b,
      ])
    ).values()
...
const busTestMode = true;
     ...
     *
     * We use sessionStorage instead of putting the complete
     * provider context into the URL.
     */
    const busSelection = {
      busId: bus.id,
  ...
  return;
    ...
    sessionStorage.setItem(
      "jetly_bus_selection",
      JSON.stringify(busSelection)
    );

    router.push(
      `/bus-passenger?busId=${encodeURIComponent(
        String(bus.id)
